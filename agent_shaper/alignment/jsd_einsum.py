"""Einsum-annotated rewrite of jsd.py.

Shape key:
  B  — batch size
  T  — sequence length
  K  — top-K vocab budget
"""
...
def compute_jsd_loss(
    ...
    if renormalize:
        student_probs = student_probs / rearrange(
            student_probs.sum(dim=-1).clamp(min=1e-8), 'b t -> b t 1'
        )                                                               # [B, T, K]
        teacher_probs = teacher_probs / rearrange(
            teacher_probs.sum(dim=-1).clamp(min=1e-8), 'b t -> b t 1'
        )                                                               # [B, T, K]
