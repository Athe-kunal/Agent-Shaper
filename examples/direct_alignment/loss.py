...
def compute_logprobs(
    ...
    logits = logits[:, :-1, :]  # (batch, seq_len-1, vocab)  # (B, T-1, V)
    labels = labels[:, 1:]      # (batch, seq_len-1)          # (B, T-1)
    mask = mask[:, 1:]          # (batch, seq_len-1)          # (B, T-1)

    # Compute per-token log probs
    log_probs = F.log_softmax(logits, dim=-1)  # (B, T-1, V)

    # Gather log prob for each target token via one-hot einsum
    # one_hot: (B, T-1, V) — 1.0 at the label index, 0 elsewhere
    one_hot = F.one_hot(labels, num_classes=log_probs.shape[-1]).float()  # (B, T-1, V)
    per_token_logps = einsum(log_probs, one_hot, "b s v, b s v -> b s")   # (B, T-1)

    # Mask out padding tokens
    per_token_logps = einsum(per_token_logps, mask, "b s, b s -> b s")    # (B, T-1)

    if average_log_prob:
        # SimPO or DPO-Norm: average over valid tokens to avoid length bias
        token_counts = reduce(mask, "b s -> b", "sum").clamp(min=1)       # (B,)
        return reduce(per_token_logps, "b s -> b", "sum") / token_counts  # (B,)
    else:
        # DPO: sum over all tokens
        return reduce(per_token_logps, "b s -> b", "sum")                 # (B,)
...
class DPOLoss(nn.Module):
    ...
    def forward(
        ...
        rejected_rewards = self.beta * rejected_logratios.detach()  # (B,)

        metrics = {
            "chosen_rewards": reduce(chosen_rewards, "b -> ", "mean").item(),
            "rejected_rewards": reduce(rejected_rewards, "b -> ", "mean").item(),
            "margins": reduce(chosen_rewards - rejected_rewards, "b -> ", "mean").item(),
            "accuracy": reduce((chosen_rewards > rejected_rewards).float(), "b -> ", "mean").item(),
        }

        return reduce(losses, "b -> ", "mean"), metrics
...
class IPOLoss(nn.Module):
    ...
    def forward(
        ...
        losses = (logits - target_margin) ** 2  # (B,)
...
class SimPOLoss(nn.Module):
    ...
    def forward(
        self,
        policy_chosen_logps: torch.Tensor,    # (B,) Already length-normalized
        policy_rejected_logps: torch.Tensor,  # (B,) Already length-normalized
        **kwargs,  # Ignore ref model logps
        ...
        logits = policy_chosen_logps - policy_rejected_logps  # (B,)

        # SimPO reference implementations treat gamma as gamma/beta ratio:
        # loss = -logsigmoid(beta * (logits - gamma_beta_ratio))
        losses = -F.logsigmoid(self.beta * (logits - self.gamma))  # (B,)

        metrics = {
            "chosen_logps": reduce(policy_chosen_logps, "b -> ", "mean").item(),
            "rejected_logps": reduce(policy_rejected_logps, "b -> ", "mean").item(),
            "margins": reduce(logits, "b -> ", "mean").item(),
            "accuracy": reduce((logits > 0).float(), "b -> ", "mean").item(),
        }
...
class ORPOLoss(nn.Module):
    ...
    def forward(
        ...
        policy_chosen_logps: torch.Tensor,    # (B,)
        policy_rejected_logps: torch.Tensor,  # (B,)
        chosen_nll_loss: torch.Tensor,        # (B,) SFT loss on chosen
        **kwargs,
    ) -> tuple[torch.Tensor, dict]:
        """Compute ORPO loss.
        ...
        policy_chosen_logps = policy_chosen_logps.float()    # (B,)
        policy_rejected_logps = policy_rejected_logps.float()  # (B,)

        # ORPO log-odds expects log-probs strictly below zero.
        policy_chosen_logps = policy_chosen_logps.clamp(max=-1e-6)    # (B,)
        policy_rejected_logps = policy_rejected_logps.clamp(max=-1e-6)  # (B,)
        ...
        chosen_rewards = self.beta * policy_chosen_logps.detach()    # (B,)
        rejected_rewards = self.beta * policy_rejected_logps.detach()  # (B,)

        # ORPO-native preference signal is the log-odds term, so log this
        # explicitly as well to reflect what ORPO actually optimizes.
        log_odds_detached = log_odds.detach()  # (B,)

        metrics = {
            "sft_loss": reduce(chosen_nll_loss, "b -> ", "mean").item(),
            "or_loss": reduce(-self.beta * ratio, "b -> ", "mean").item(),
            "log_odds_ratio": reduce(log_odds, "b -> ", "mean").item(),
            "log_odds_accuracy": reduce((log_odds_detached > 0).float(), "b -> ", "mean").item(),
            "chosen_logps": reduce(policy_chosen_logps, "b -> ", "mean").item(),
            "rejected_logps": reduce(policy_rejected_logps, "b -> ", "mean").item(),
            "margins": reduce(chosen_rewards - rejected_rewards, "b -> ", "mean").item(),
            "accuracy": reduce((chosen_rewards > rejected_rewards).float(), "b -> ", "mean").item(),
        ...
        return reduce(loss, "b -> ", "mean"), metrics
    ...
    def forward(
        ...
        if policy_kl_logps is not None and ref_kl_logps is not None:
            ...
            if batch_size > 1:
                ...
                shifted_rejected = rejected_logratios.roll(shifts=1, dims=0)  # (B,)
        ...
        chosen_losses = 1 - F.sigmoid(self.beta * (chosen_logratios - KL))    # (B,)
        # Undesirable: want rejected to be worse than reference point
        rejected_losses = 1 - F.sigmoid(self.beta * (KL - rejected_logratios))  # (B,)

        # Weighted combination (asymmetric weights reflect loss aversion)
        loss = (
            self.desirable_weight * reduce(chosen_losses, "b -> ", "mean")
            + self.undesirable_weight * reduce(rejected_losses, "b -> ", "mean")
        )

        metrics = {
            "KL": KL.item(),
            "chosen_logratios": reduce(chosen_logratios, "b -> ", "mean").item(),
            "rejected_logratios": reduce(rejected_logratios, "b -> ", "mean").item(),
            "chosen_loss": reduce(chosen_losses, "b -> ", "mean").item(),
            "rejected_loss": reduce(rejected_losses, "b -> ", "mean").item(),
            "accuracy": reduce((chosen_logratios > rejected_logratios).float(), "b -> ", "mean").item(),
        }
...
class APOZeroLoss(nn.Module):
    ...
    def forward(
        ...
        """
        # Compute log ratios (implicit rewards)
        chosen_logratios = self.beta * (policy_chosen_logps - ref_chosen_logps)    # (B,)
        rejected_logratios = self.beta * (policy_rejected_logps - ref_rejected_logps)  # (B,)
        # If `chosen_logratios` increases -> loss decreases
        # If `rejected_logratios` decreases -> loss decreases
        apo_zero_loss = -F.sigmoid(chosen_logratios) + F.sigmoid(rejected_logratios)  # (B,)
        chosen_rewards = chosen_logratios.detach()    # (B,)
        rejected_rewards = rejected_logratios.detach()  # (B,)
        metrics = {
            "chosen_rewards": reduce(chosen_rewards, "b -> ", "mean").item(),
            "rejected_rewards": reduce(rejected_rewards, "b -> ", "mean").item(),
            "margins": reduce(chosen_rewards - rejected_rewards, "b -> ", "mean").item(),
        ...
        return reduce(apo_zero_loss, "b -> ", "mean"), metrics
...
class APODownLoss(nn.Module):
    ...
    def forward(
        ...
        return reduce(apo_down_loss, "b -> ", "mean"), metrics
