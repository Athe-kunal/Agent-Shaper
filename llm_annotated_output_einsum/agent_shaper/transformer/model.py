...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # q, k, v: (batch, time, head, head_dim)
        ...
        if self.flash:
            ...
            # q, k, v: (batch, head, time, head_dim)
            ...
            # y: (batch, time, head, head_dim)
            y = y.transpose(1, 2)
        else:
            # manual implementation of attention
            # att: (batch, head, query_time, key_time)
            att = torch.einsum('bthd,bshd->bhts', q, k) * (1.0 / math.sqrt(d_head))
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # x: (b, t, d_model)
        x = torch.einsum('btd,od->bto', x, self.c_fc.weight) + (self.c_fc.bias if self.c_fc.bias is not None else 0.0)
        # x: (b, t, d_hidden)
        x = self.gelu(x)
        x = torch.einsum('bto,do->btd', x, self.c_proj.weight) + (self.c_proj.bias if self.c_proj.bias is not None else 0.0)
        # x: (b, t, d_model)
        x = self.dropout(x)
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos = torch.arange(0, t, dtype=torch.long, device=device)  # pos indices
